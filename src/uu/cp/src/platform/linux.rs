...
/// Copy sparse data up to a size snapshot, shrinking the destination if the
    ...
    P: AsRef<Path>,
{
    ...
    let blksize: usize = dst_file
        .metadata()
        .map_err(&ctx_err)?
        .blksize()
        .try_into()
        .map_err(|error| ctx_err(std::io::Error::new(std::io::ErrorKind::InvalidData, error)))?;
